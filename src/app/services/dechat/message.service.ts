...
export class MessageService {
  ...
  async setCurrentChat(chat: ChatInfo) {
    ...
    var bundles = this.chatMap.get(this.currentChat);
  ...
  private pushMessage(msg : ChatMessage) {
    ...
    this.currentChat.users.forEach(async user => {
      var chatPath = await this.files.getChatUrl(this.user, this.currentChat);
      var path = chatPath + this.currentBundle.bundleId
  ...
  private async fetchMessageBundle(bundleUrl: string) {
    ...
    var id = bundleUrl.replace(this.currentChatUrl, "").replace("/", "");
  ...
  private createBundle(id : string) : MessageBundle {
    var bundle = new MessageBundle(this.currentChat, id);
