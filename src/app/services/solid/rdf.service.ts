...
export class RdfService {
    ...
    /**
     * This method is used for extracting the value of a field from the user's profile.
     * @param field Field to extract from user's profile.
     */
    async getProfileField(field: string): Promise<string> {
        return this.getFieldString(this.session.webId, field, VCARD);
    }

    /**
     * This method is used for extracting the value of a field and to return it as a string.
     * @param webId webId from where the information is going to be extracted.
     * @param field field to be extracted.
     * @param namespace Ontology to use.
     */
    async getFieldString(webId: string, field: string, namespace: any): Promise<string> {
        try {
     ...
     * @param namespace Ontology to use.
     */
    async getFieldArray(webId: string, field: string, namespace: any): Promise<Array<NamedNode>> {
        try {
            await this.fetcher.load(this.store.sym(webId).doc());
            return this.store.any(this.store.sym(webId), namespace(field));
        } catch (err) {
    ...
    async getContacts(): Promise<Array<NamedNode>> {
        ...
        return this.getFieldArray(webId, 'knows', SCHEMA);
    }

    async getMessages(fileUrl: string): Promise<Array<NamedNode>> {
        return this.getFieldArray(fileUrl, 'Message', SCHEMA);
    }

    async getUserData(webId: string, field: string): Promise<string> {
        return this.getFieldString(webId, field, VCARD);
    }

    addFriend(webId: string) {
        const user = $rdf.sym(this.session.webId);
        const friend = $rdf.sym(webId);
        const toInsert = $rdf.st(user, FOAF('knows'), friend, user.doc());
        this.updateManager.update([], toInsert, (response, success, message) => {
            if (success) {
